    ...
    while (bRunning)
    {
        ...
        if (Embd.size() > 0)
        {
            ...
            for (int i = 0; i < (int)Embd.size(); i += NBatch)
            {
                ...
                if (bShouldLog)
                {
                    std::string DetokenizedString = std::string{};
                    for (auto j = 0; j < NEval; ++j)
                    {
                        DetokenizedString += llama_detokenize(Context, { Embd[i + j] });
                    }
                    UE_LOG(LogTemp, Warning, TEXT("%p eval tokens `%s`"), this, UTF8_TO_TCHAR(DetokenizedString.c_str()));
                }
        ...
        if ((int)EmbdInput.size() <= NConsumed)
        {
...

            {
                ...
                int LastNRepeat = FMath::Min(FMath::Min((int)LastNTokens.size(), P.RepeatLastN), NCtx);
...
void FLlama::UnsafeActivate(bool bReset) 
{
    ...
    // tokenize the Prompt
    std::string StdPrompt = std::string(" ") + TCHAR_TO_UTF8(*Params.Prompt);
    ...
    if (!Params.StopSequences.IsEmpty())
    {
        for (int i = 0; i < Params.StopSequences.Num(); ++i)
        {
            ...
            std::string str = std::string{ TCHAR_TO_UTF8(*stopSeq) };
            if (::isalnum(str[0]))
                str = " " + str;
            std::vector<llama_token> seq = my_llama_tokenize(Context, str, Res, false /* add bos */);
