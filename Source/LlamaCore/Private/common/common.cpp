...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "--tensor-split" || arg == "-ts") {
        ...
        for (size_t j = 0; j < llama_max_devices(); ++j) {
            if (j < split_arg.size()) {
                params.tensor_split[j] = std::stof(split_arg[j]);
            }
            else {
    ...
    while ((pos_slash = path.find('\\', pos_slash)) != std::string::npos) {
        ...
        if (!success) {
            ...
            if (error == ERROR_ALREADY_EXISTS) {
                const DWORD attributes2 = GetFileAttributesW(subpath.c_str());
                if (attributes2 == INVALID_FILE_ATTRIBUTES || !(attributes2 & FILE_ATTRIBUTE_DIRECTORY)) {
                    return false;
                }
            } else {
                return false;
